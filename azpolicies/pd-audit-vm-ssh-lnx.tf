resource "azurerm_policy_definition" "pd-audit-vm-ssh-lnx" {
  name         = "pd-audit-vm-ssh-lnx"
  policy_type  = "Custom"
  mode         = "All"
  display_name = "Audit Linux Virtual Machines must require SSH keys for authentication"
...
output "policydefinition_pd-audit-vm-ssh-lnx" {
  value = azurerm_policy_definition.pd-audit-vm-ssh-lnx
}